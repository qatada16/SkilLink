...
const PostJob = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error("Error posting job:", error);
      setStatusMessage(error.response?.data?.message || "Failed to post job. Try again.");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-[#1a1a1a] p-8 rounded-xl shadow-2xl">
          ...
          <button 
            ...
            className={`w-full py-3 px-4 ${
              isLoading ? 'bg-gray-500' : 'bg-[#1abc9c] hover:bg-[#16a085]'
            } text-white rounded-lg font-semibold transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-[#1abc9c] focus:ring-offset-2 focus:ring-offset-[#1a1a1a]`}
          >
            {isLoading ? 'Posting...' : 'Post Job'}
          </button>
          {statusMessage && (
            <p className={`text-center ${
              statusMessage.includes("successfully") ? "text-[#1abc9c]" : "text-red-500"
            }`}>
              {statusMessage}
            </p>
          )}
