...
//     const { jobID, price, cID, freelancerID, proposalID } = jobResult.recordset[0];
...
router.post("/complete/:jobID", async (req, res) => {
  ...
  try {
    ...
    // Get job details including price, client ID, and freelancer ID
    const jobQuery = `
      SELECT j.price, j.cID, p.freelancerID, p.proposalID
      FROM Jobs j
      JOIN Proposals p ON j.jobID = p.jobID
      WHERE j.jobID = @jobID AND p.pStatus = 'Accepted'
    `;
    ...
    // Changed variable name to avoid conflict with existing jobID
