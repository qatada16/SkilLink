...
CREATE TABLE Messages (
    ...
    content NVARCHAR(MAX) NOT NULL,  -- Changed from TEXT to NVARCHAR(MAX)
    timestamp DATETIME NOT NULL DEFAULT GETDATE(),  -- Changed from sentOn smalldatetime
    isRead BIT NOT NULL DEFAULT 0,
    -- Removed jID as it's not used in any of the routes
    -- Added indexes for better performance on common queries
    INDEX IX_Messages_SenderReceiver NONCLUSTERED (senderID, receiverID),
    INDEX IX_Messages_ReceiverSender NONCLUSTERED (receiverID, senderID),
    INDEX IX_Messages_Timestamp NONCLUSTERED (timestamp)
);
...
INSERT INTO Jobs (cID, Title, description, targetSkills, connectsRequired, estTime, jobLevel,price)VALUES
(13, 'Prob Assignment', 'Prob ass', 'math, ct', 1, '1 weeks', 'Beginner',10)
GO
